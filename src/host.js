...
const ANNOUNCEMENT_ZH = [
  ...
  "质量底线（每份问卷，无例外）——1. 导语定锚：这决定什么、为何是现在、答案怎么用、大约多久。2. 自包含题干：术语就地定义、说明适用位置与答案会改变什么——错过上下文的读者也能独立看懂。3. 每题 ≥5 个有立场差异的选项：读者真的可能选的立场——绝不凑数，绝不用「选项A」；自由文本行是逃生门，不算选项。4. 洞察行：每个判断型选项的 insight 排成 2-4 行（markdown 列表，一行一个加粗标签）——**Proven:** 已被成功使用的模式 + 在哪里使用 · **Costs:** 放弃什么、冒什么险（一行）· **Now:** 今天站在哪里，具体抓手（数字、名字、版本、日期）写在 Now 行内。写成一段的洞察、或没有行的洞察，是你制造的盲选。5. 有分支或说明理由：某个答案理应改变后续提问时，必须有分支；答案明显重要却拍平成序列就是缺陷。6. 语言一致：所有用户可见文案用用户当前聊天语言（中文对话 → 中文问卷）；选项 key 一律短 ASCII（a、b、c…）。7. 比例原则：极小确认可以更轻（2-4 个选项、短题干）——但自包含题干与诚实选项是处处生效的底线。",
...
function surveyToolDefinition(ctx, service) {
  const optionSchema = {
    ...
    properties: {
      ...
      insight: { type: 'string', description: 'Markdown revealed on hover (~6 lines), 2-4 markdown ROWS, one bold-labeled point per row: **Proven:** the pattern used with success + the named system or file where it works; **Costs:** what it costs or risks; **Now:** where things stand today, with the concrete handle (a number, a name, a version, a date) in the Now row. No paragraphs.' },
  ...
  const quickOptionSchema = {
    ...
    properties: {
      ...
      description: { type: 'string', description: 'One sentence shown under the label (always visible).' },
      insight: { type: 'string', description: 'Markdown revealed via the "?" button (~6 lines), 2-4 markdown ROWS, one bold-labeled point per row: **Proven:** what this template’s stance is and where it’s proven; **Costs:** its tradeoff; **Now:** who it is for today.' },
...
function draftToolDefinitions(ctx, service, structureQuestionCap) {
  ...
  const getTool = {
    ...
    description: 'Builder read and the LAUNCH GATE: returns the draft (by slug, else the conversation active draft) with the required-field checklist — every option needs label, description, insight (2-4 ROWS: Proven / Costs / Now), and at least 1 source; every question needs a non-TODO self-contained prompt. Loop [research → survey_draft_set op=patch] until this reports zero gaps; only then call survey_draft_launch.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
