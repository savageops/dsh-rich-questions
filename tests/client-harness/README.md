...
`drive.mjs` loads an actual bundle build into a jsdom window, activates the plugin through the same `ctx.slots` seam the host uses, hydrates the survey store through the `/state` poll path, then walks a real spec: start page, question pages with option selects, branch follow, insight expansion, back navigation, quick screen with a template pick. It then drives the two crash classes that each unmounted the conversation composer in production:
